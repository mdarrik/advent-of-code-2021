...
struct BingoGame<'a> {
    ...
    number_of_boards_in_play: usize,
    losing_board: Option<Board>,
...
impl BingoGame<'_> {
    fn parse_game_input(input: &str) -> BingoGame {
        ...
        let mut boards_in_play: HashMap<usize, bool> = HashMap::new();
        ...
        for line in lines {
            ...
            if line == "" {
                ...
                boards_in_play.insert(board_index, true);
                board = Board::default();
                board_index += 1;
                row_num = 0usize;
                continue;
        ...
        BingoGame {
            ...
            boards,
            value_board_map,
            number_of_boards_in_play: board_index + 1,
            losing_board: None,
        ...
        for (index, number_str) in self.drawn_numbers.clone().into_iter().enumerate() {
            ...
            if let Some(boards_with_value) = self.value_board_map.get(&number) {
                    ...
                    boards_with_value
                {
                    ...
                    // if the board is not in play, skip
                    if !matching_board.in_play {
                        continue;
                    }
                    matching_board
                        .marked
                    ...
                    if (index >= matching_board.num_rows
                        && matching_board.did_row_win(row_position))
                        || (index >= matching_board.num_cols
                            && matching_board.did_col_win(col_position))
                    {
                        if self.number_of_boards_in_play > 1 {
                            self.number_of_boards_in_play -= 1;
                            matching_board.in_play = false;
                        } else {
                            self.last_drawn_number = Some(number);
                            self.losing_board = Some(matching_board.clone());
    ...
    fn play(&mut self) -> u32 {
        self.draw_numbers();
        if let Some(board) = &self.losing_board {
